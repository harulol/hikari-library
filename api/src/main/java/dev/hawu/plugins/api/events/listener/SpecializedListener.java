...
public final class SpecializedListener<T extends Event> implements ClosableListener {
     ...
     * @since 1.0
     */
    public SpecializedListener(@NotNull final JavaPlugin plugin, final int invocationExpiry, final double timedExpiry,
        ...
        if(timedExpiry > 0) Tasks.scheduleLater(plugin, (long) timedExpiry, runnable -> this.close());
    }
    ...
    @EventHandler
    private void onEvent(@NotNull final T event) {
