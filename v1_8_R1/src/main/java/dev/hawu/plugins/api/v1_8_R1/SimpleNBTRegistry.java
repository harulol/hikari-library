package dev.hawu.plugins.api.v1_8_R1;
...
/**
 * Extension for the NBT registry for Bukkit v1_8_R1.
 *
...
public final class SimpleNBTRegistry extends NBTRegistry {
    ...
    private NBTList transformNMSList(@NotNull final NBTTagList list) {
        ...
        for(int i = 0; i < list.size(); i++) {
            final NBTType nbt = transformNMSBase(list.g(i));
    ...
    private NBTCompound transformNMSCompound(@NotNull final NBTTagCompound compound) {
        ...
        for(final Object obj : compound.c()) {
            if(!(obj instanceof String) || compound.get((String) obj) == null) continue;
            final NBTType nbt = transformNMSBase(compound.get((String) obj));
            if(nbt != null) tag.put((String) obj, nbt);
        }
        return tag;
    }
    ...
    public @Nullable NBTCompound getCompound(final @NotNull ItemStack item) {
        final net.minecraft.server.v1_8_R1.ItemStack nmsCopy = CraftItemStack.asNMSCopy(item);
        if(nmsCopy.hasTag())
